...
4. ✅ **ADR-106 — одобрение пользователя как граница автоматизации** (2026-08-19): опт-ин =
   одобрение по умолчанию для всех активностей (каталог/профиль/привнесённые);
   `risk_level`/`automation_allowed` — информационные метаданные, риск-гейт REM §5.2 в рантайме
   отменён (`filter_automation_eligible` → passthrough). Остаются жёсткими: stop/отказ всегда,
   медиа не обязательно, без штрафа за отказ, без обхода safety-фильтров, без авто-эскалации.
...
- Автоматизация research-карточек и не импортированных источников (их нет в исполняемом каталоге).
