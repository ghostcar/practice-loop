...
|---|---|---|
| **E — Сущность/состав/автопоиск/группировка** | миграция 097, модели, сервис нормализации и синхронизации компонентов, `pharma_enricher` (seed+маска Фемостон+LLM), форма с составом, карточка, группировка «по веществу», поиск, i18n, тесты | ✅ реализовано (2026-09-04, коммит 5f82bf82+след.) |
| **F — Замена** | автовыбор по составу+форме (пересчёт мг), «Эквиваленты», `substituted_for_id`, JSON parity | ✅ реализовано (2026-09-04) |
| **G — Пределы и схемы пачек** | сверка суточных доз (предупреждение/разрешение), учёт вариантов в дне | ✅ реализовано (2026-09-04; курсы/напоминания — точечно, см. §10c) |

## 10a. Факт реализации фазы E (2026-09-04)

- Миграция `097_substances_components` (roundtrip на реальном PostgreSQL + backfill legacy
  `active_ingredient` → вещество/компонент проверен на данных).
- Модели: `MedSubstance` (norm_key UNIQUE, inn, synonyms, daily_max_*, is_custom),
  `MedVariant` (таблетки пачки), `MedComponent` (med ↔ substance + amount/unit/variant);
  `medications.allow_ul_override`.
- Сервис: `normalize_substance`, `find_or_create_substance`, `sync_med_components`,
  `parse_components_payload` (JSON-строка формы и list JSON API; дедуп «вещество+вариант»),
  подпись состава `composition_label`, поиск по name/компонентам/МНН/синонимам (`q`),
  группировка «по действующему веществу» на странице.
- Автопоиск починен (`pharma_enricher` переписан): structured-seed с INN и daily_max_*,
  маска «Фемостон N/M»/«конти» с вариантами пачки, реальный LLM-разбор (`call_llm`, json_mode,
  json_repair), честный «не найдено» вместо мусорного автозаполнения; JSON parity
  `POST /api/v2/medications/autofill` (404 = не найдено).
- Форма: редактор строк «вещество + доза + единица (+ вариант)» с datalist, чекбокс
  «Разрешено превышение суточной дозы», автозаполнение заполняет состав.
- Тесты: +10 (53 медикаментозных зелёных), весь набор **1510 passed / 4 skipped**;
  ruff чистый; i18n parity (EN/RU) сохранена.

## 10b. Факт реализации фаз F/G (2026-09-04)

- Миграция `098_substitution_daily_limit` (roundtrip на реальном PostgreSQL):
  `med_intakes.substituted_for_id` (FK medications, SET NULL, index) +
  `med_intakes.ul_confirmed`.
- **Замена (F)**: `equivalent_candidates()` — тот же набор компонентов (norm_key) + та же
  форма → `auto` с пересчётом по мг (1×400 = 2×200); мг не парсится → авто при равенстве
  строки strength (включая «обе пустые»); иной набор/форма/вид → `offer` с показом состава
  и остатков по аптечкам (локации). На «сегодня» каждый due/slot получает `stock_total`,
  `stock_needed_today`, `insufficient` и `substitute` (первый авто с `qty_per_intake`).
  Приём заменителем: `medication_id` = фактический, `substituted_for_id` = оригинал,
  автотекст «Замена: …» в notes. Эндпоинты: `GET /medications/{id}/equivalents`,
  `GET /api/v2/medications/{id}/equivalents`, intake-form/JSON принимают фактический препарат;
  блок «Эквиваленты» на карточке.
- **Пределы (G)**: `daily_limit_exceedances()` — сумма компонентов на день (с учётом
  варианта пачки `variant_for_day()`: Фемостон 2/10 — белые 1–14/серые 15–28, цикл от
  start_date; единицы мкг/мг/г + МЕ↔мкг для известных веществ) vs `daily_max_*`;
  превышение → `limits.warnings` (есть препарат без разрешения) или `limits.allowed`
  (все препараты вещества с `allow_ul_override`); на due-строках флаги `exceeds_daily`/
  `ul_pending` и кнопка «Принять с превышением» (`ul_confirmed` фиксируется в intake).
  Блок «Контроль суточной дозы» на странице; JSON parity через `GET /api/v2/medications/today`
  (ключ `limits`).
- Тесты: +6 (59 медикаментозных зелёных), весь набор **1516 passed / 4 skipped**;
  ruff чистый; i18n parity (EN/RU) 1724/1724.

## 10c. Факт реализации: «Поиск аналогов ИИ» — реальный LLM-пайплайн (2026-09-04)

Заглушка ADR-109 («Дженерик …», «ФармаЛайн») заменена реальным поиском по составу
через BYOK-LLM (принципы ADR-190: ничего не выдумывается, пустой список = честное
«не найдено»):

- **`find_analogs()`**: состав берётся из `composition_label()` (компоненты) или legacy
  `active_ingredient`; пустой состав → `no_composition`; `get_active_llm_config(capability="text")`
  → `no_llm`; `call_llm(json_mode)` + `parse_llm_json` → `llm_error` при сети/сбое.
- **Санитизация**: только заполненные имена, дедуп по `normalize_substance`, исключение
  исходного препарата, максимум 8 записей; неизвестные поля → null; разметка «уже в
  аптечке» (`owned`) по нормализованным названиям препаратов пользователя.
- **UI**: страница-форма POST → redirect на карточку `#med-{id}` с ошибкой
  (`analogs_error=no_composition|no_llm|llm_error` — локализованный баннер) или
  `analogs_done=1`; результаты на карточке: summary, бейджи «тот же состав»/«уже в
  аптечке», производитель/форма/дозировка, notes, дисклеймер; пустой результат —
  «ИИ не нашёл известных аналогов».
- **JSON parity**: `POST /api/v2/medications/{id}/analogs` (404/422/502).
- Тесты: +6 (LLM-мок с санитизацией/дедупом/owned, no_llm, no_composition, ошибка LLM,
  мусорный JSON, redirect с ошибкой). Медикаментозные 65 зелёных, весь набор
  **1522 passed / 4 skipped**; ruff чистый.
