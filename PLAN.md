...
- [ ] **Шаг 9 — Редизайн фронта по DESIGN_V2.md** — «Тёмный архив»: новый UI-контракт (темы, типографика, компоненты, навигация) поверх существующих экранов, с соблюдением обязательства по икон-паку. Запланирован по решению владельца.
    ...
    - ⏳ **9a — остаточный долг a11y (на 9d/9f)**: axe-тест `@a11y` на desktop-chromium падает на legacy-контрастах в тёмной теме: `a.text-indigo-500/-400` ссылки (#615fff 3.75:1), `text-slate-400 dark:text-slate-500` тексты-пустышки (#62748e 3.65:1), `pl-accent-text` XP-процент (accent #a95f58 3.63:1) на surface. Чинится токенизацией в 9d/9f; тест начнёт проходить после. `prototype.spec.ts` скипается осознанно (нужен DESIGN_PROTOTYPE_URL).
