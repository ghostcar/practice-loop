...
- [ ] **Шаг 5 — Стабилизация поведения (Gate B остаток)** — единый владелец транзакций (сервисы коммитят), асинхронная обработка медиа (поток/очередь, защита от decompression bomb), браузерные smoke-тесты (RU/EN × светлая/тёмная × 360/768/1280: логин, дашборд, задачи, таймер).
