...
**Политика ADR-137** (смягчение PD-013, ADR-085): своевременный приём может
давать XP и достижения — **положительное подкрепление**, настраивается на
пользователя (prefs.med_gamification, default ON). Пропуск/мисс НИКОГДА не
отнимает баллы и не штрафуется (негативная геймификация здоровья запрещена).
Медицинский сигнал остаётся relief-only (открыть окно/смягчить/пауза/стоп).
Все записи Private Record (DATA_LIFECYCLE.md).
