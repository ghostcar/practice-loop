...
(function () {
  ...
  (async function () {
    try {
      ...
      listEl.innerHTML = '<div class="p-4 text-sm text-red-400">Failed to load locations</div>';
    }
  })();
  ...
  window.renderList = function () {
    ...
    listEl.innerHTML = items.map(rowHTML).join('') ||
      '<div class="p-4 text-sm text-slate-400 dark:text-slate-500">No locations found</div>';
  };
  ...
  function rowHTML(loc) {
    ...
    var icons = loc.is_custom
      ? '<button onclick="window._locEdit(\'' + loc.id + '\')\" class="text-slate-400 hover:text-indigo-500 dark:hover:text-indigo-400 p-1">\u270e</button>' +
        '<button onclick="window._locArchive(\'' + loc.id + '\')\" class="text-slate-400 hover:text-amber-500 p-1">\ud83d\udce6</button>' +
        '<button onclick="window._locDelete(\'' + loc.id + '\')\" class="text-slate-400 hover:text-red-500 p-1">\u2715</button>'
      : '';
