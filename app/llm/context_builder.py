"""Context Builder: gathers user history, stats, active penalties, and allowed entities.
...
async def build_context(
    ...
    """Build the full LLM context: allowed entities, recent history, stats."""
...
async def _get_allowed_entities(db: AsyncSession, user_id: uuid.UUID) -> list[dict]:
    """Return entities the user has opted into, with desire levels."""
...
async def _get_active_penalties(db: AsyncSession, user_id: uuid.UUID, session_id: uuid.UUID | None) -> list[dict]:
    ...
    # Count ALL interrupted tasks (escalation builds from total interruptions)
...
def format_context_for_prompt(context: dict) -> str:
    ...
    for e in context["allowed_entities"]:
        ...
        parts.append(f"- [{desire}] {e['name']} (id={e['id']}, category={e['category']}, type={e['type']})")
