...
def upgrade() -> None:
    ...
    op.add_column("lock_slot_rules", sa.Column("chastity_device_id", UUID(as_uuid=True), sa.ForeignKey("inventory_items.id", ondelete="SET NULL"), nullable=True))
    op.add_column("lock_slot_rules", sa.Column("keyholder_type", sa.String(length=30), server_default="llm_bot", nullable=False))
    op.add_column("lock_slot_rules", sa.Column("auto_pause_on_health_drop", sa.Boolean(), server_default="true", nullable=False))
    op.add_column("lock_slot_rules", sa.Column("extension_history", JSONB(astext_type=sa.Text()), nullable=True))

    # Upgrade lock_sessions
    op.add_column("lock_sessions", sa.Column("chastity_device_id", UUID(as_uuid=True), sa.ForeignKey("inventory_items.id", ondelete="SET NULL"), nullable=True))
    op.add_column("lock_sessions", sa.Column("keyholder_type", sa.String(length=30), server_default="llm_bot", nullable=False))
