...
| `/llm-configs/` | BYOK-конфиги провайдеров, активный конфиг, режимы full/abstract, хранение raw; доступен во всех вариантах продукта; пункт в главной навигации (ADR-191); журнал вызовов LLM `/llm-configs/logs` (ADR-191) |
...
- **Механизм аптечек, режимов и курсов (ADR-189)**: **фазы A+B+C реализованы** — всегда-видимая секция аптечек с карточками (имя/локация/препараты/срок годности), в форме препарата select «Аптечка» + «Остаток» → авто-создание партии; режим приёма — семантические параметры `food_relation`, `duration_days` (end_date вычисляется), `meal_timing`, `meal_offset_min` (миграция 095), пресеты `REGIMEN_PRESETS` (кнопки-чипы), живой текст-превью режима, meal-based времена через `schedule_times`; **курсы (миграция 096)** — `med_courses` (planned/active/paused/completed) + элементы = расписания с `course_id` (каждый со своим режимом), секция «Курсы приёма» на странице (создание курса, добавление препарата с режимом, статус, `course_summary`: потребление на курс vs партии по аптечкам с иерархическими локациями + план-превью), **сводный приём**: в «На сегодня» все препараты курсов/расписаний группируются в одну задачу на временной слот (`med_group` + batch `POST /med-intakes/batch`), аптечки привязываются к дереву локаций (`med_kits.location_id`, legacy text-подпись сохранена как fallback); **умный ввод (фаза D)** — поле «Напишите как принимать» + кнопка «Разобрать» в формах расписания и элемента курса: свободный текст («3 раза в день до еды 20 дней») → детерминированный rule-парсер RU/EN `parse_regimen_text` (число раз, до/после/во время еды, натощак, каждые N часов, N дней, по дням недели, с даты, явные времена, доза) → параметры заполняют форму → живой превью → подтверждение перед сохранением (human-in-the-loop); `POST /medications/parse-regimen` (страница) и JSON `POST /api/v2/medications/regimen/parse` (mobile parity); парсер ничего не сохраняет. Спека: `docs/wiki/K-MEDICATION-COURSES-SPEC.md`.
- **Механизм «активного элемента», состава и замены (ADR-190)**: действующее вещество отделяется от МНН и становится канонической сущностью `med_substances` (name, norm_key UNIQUE, inn — МНН хранится здесь, synonyms, daily_max_* — суточный предел, is_custom; системный seed из `pharma_enricher`). Препарат = 1..N **компонентов** (`med_components`: вещество + дозировка/единица + вариант пачки `med_variants` — Фемостон 2/10, витаминные комплексы); `medications.allow_ul_override` — явное разрешение превышения суточной дозы (сверка — фаза G). Группировка «по действующему веществу» и поиск (название/вещество/МНН/синонимы) на странице; карточка показывает состав. При нехватке остатка на «сегодня» — замена (F ✅): автовыбор при том же наборе компонентов + форме (пересчёт дозы по мг: 1×400 = 2×200; `med_intakes.substituted_for_id` + автотекст «Замена»), иначе предложение с эквивалентами и остатками по аптечкам; блок «Эквиваленты» на карточке; JSON parity `GET /api/v2/medications/{id}/equivalents`. Суточные пределы (G ✅): сверка суммы компонентов дня (с учётом вариантов пачки — Фемостон 1–14/15–28) vs `daily_max_*`; превышение — блок «Контроль суточной дозы» и кнопка «Принять с превышением» (`ul_confirmed`); `allow_ul_override` на препарате выводит превышение в «разрешённые». **Автопоиск по наименованию починен (фаза E ✅)**: structured-seed (INN/daily_max) + маска «Фемостон N/M», реальный LLM-разбор (BYOK, json_mode, repair), честный «не найдено» вместо мусора; заполняет форму, включая состав. **«Поиск аналогов ИИ» — реальный LLM-пайплайн (ADR-109 ✅)**: поиск дженериков/аналогов по составу через BYOK-LLM (json_mode + repair, строгая санитизация: только реальные имена, дедуп, макс. 8; пустой список = честное «не найдено»), разметка «уже в аптечке», страница-форма с редиректом на карточку (`analogs_error`/`analogs_done`), JSON parity `POST /api/v2/medications/{id}/analogs`. Дизайн и фазы: `docs/wiki/K-MEDICATION-SUBSTANCES-SPEC.md`. **Карточка препарата — просмотр и редактирование (ADR-191 ✅)**: якорь `#med-{id}`, кнопка «Открыть карточку» (детальный блок), форма редактирования (наименование/производитель/форма/дозировка/вид/аптечка/заметки/состав с веществами/`allow_ul_override`) → `update_medication`, редирект на якорь; состав редактируется целиком, вещества переиспользуются.
- **Журнал вызовов LLM (ADR-191 ✅)**: таблица `llm_call_logs` (миграция 099; метаданные без raw — ADR-034), запись на каждый вызов `call_llm` и всех call-сайтов (agent/*, locktimer, pipeline/*); страница `/llm-configs/logs` (фильтр ok/error, статистика токенов/стоимости, ошибки видны пользователю), JSON parity `/llm-configs/logs/json` и `/api/v2/llm/logs`. Вместе с фиксами: `/admin/llm-pool` (eager-load моделей, дубликат имени → локализованная ошибка) и фолбэк устаревшего portal id в resolver.
