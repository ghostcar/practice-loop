...
- **Механизм «активного элемента» и замены (ADR-190)**: действующее вещество отделяется от МНН и становится канонической сущностью `med_substances` (name/norm_key/inn/synonyms, seed из `pharma_enricher.LOCAL_PHARMA_SEED`); `medications.substance_id` — ссылка на сущность, МНН показывается из `substance.inn`; группировка/поиск/замена по сущности; при нехватке остатка на «сегодня» автовыбор препарата с тем же активом и формой (пересчёт количества по дозировке в мг, `med_intakes.substituted_for_id` фиксирует замену), иная форма — предложение с эквивалентами и остатками. Дизайн: `docs/wiki/K-MEDICATION-SUBSTANCES-SPEC.md` (фазы E–G).
