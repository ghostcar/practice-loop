...
прикрепляются к карточкам как `alternate_names` (226/286) по curated-правилам + транслитерированному
...
### Фильтр фитнес/трекер-шума

55 названий-шума помечаются `noise=true` + `seed_ready=false` и исключаются из пула синонимов —
никогда не попадают в `alternate_names`. Это фитнес-программы («Планка», «Йога для похудения»,
«Пресс за 30 дней», «Упражнение для растяжки…», «Фитнесс для женщин…») и трекерные строки
(«Вес утро/вечер», «Объём груди/талии/бёдер», «Бонус», «Штраф», «КП», «Тело», «Подготовка»).
Ранее token-overlap привязывал их к несвязанным карточкам («Тренировка ягодиц» → bowel control,
«Объём груди» → chest impact). Оставшиеся однословные кинк-термины («Упаковка», «Питьё», «ЗД»,
«Опорожнение», «Уборка») привязаны к карточкам точечно через `DIRECT_SYNONYM_MAP` (точный title_id,
без substring-матчинга): «Упаковка» → post-release-internal-wear, «Питьё» → funnel-urine-pour,
«ЗД» → fecal-retention, «Опорожнение» → controlled-partial-release, «Уборка» → post-scene-cleanup-service.
Итог: все нешумовые названия привязаны (226 + 5 прямых = 231/231), непривязанных нет.

### Extension cards (ADR-119, четыре категории без 163-записей)

`data/seed/adult_activity_extensions.v1.json` — 33 карточки для категорий, которых не было в
163 записях источника: `humiliation_objectification` (7), `service_protocol` (8),
`psychological_control` (11), `clothing_fetish` (7). Каждая карточка привязывает свои
`rules` к соответствующим дополнительным названиям (`source_refs` = additional-title IDs,
`alternate_names` = названия). Генератор строит их первыми, чтобы широкие токены
(compression, watersports) не уводили названия в соседние карточки. Итог каталога:
**187 карточек** (34 candidates + 120 promoted + 33 extension), все kink-названия привязаны.
