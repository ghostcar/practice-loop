...
| 2026-08-10 | Session 48 | CSRF fix: `verify_csrf` (app/security.py) стал async и валидирует `csrf_token` из поля формы (double-submit) — нативные POST-формы темы/локали ломались с 403 «CSRF token missing or invalid» (проверялся только HTMX-заголовок). Парсинг только form content-type; `request.body()` перед `form()` (Starlette 1.4.1 body-replay, иначе 422). main.py: `await verify_csrf`. +3 теста (тема/локаль/неверный токен) — **228/228 тестов ✅**, ruff ✅ |
