...
ФОРМАТ РЕЕСТРА (канонический, обязателен для новых ADR):
  ...
  Номера без реестровой строки, но упомянутые в коде/доках (ADR-120, 121, 126–134, 145, 159),
  так же считаются неиспользованными — на них не ссылаются как на принятые решения.
...
- **Коммит:** (pending)
| ADR-172 | 2026-08-25 | Service layer extraction: diets.py → diets_service.py (529→386 thin routes, 422 service) | thin routes; коммит 1fc7b628 | принято |
| ADR-173 | 2026-08-25 | Service layer extraction: prompt_templates.py → prompt_templates_service.py (505→307 thin routes, 350 service) | thin routes; коммит 085e9a85 | принято |
...
| ADR-177 | 2026-08-25 | Initial onboarding wizard — 3-step flow (superseded by ADR-184) | feature; коммит 07421c75 | принято |
| ADR-178 | 2026-08-25 | Service layer extraction: communities/admin/catalog/community_agent/aftercare (5 files, ~1713 lines total) | thin routes; коммит 1d5e74b6 | принято |
<!-- ADR-179: номер не использован (пропущен при нумерации). -->
