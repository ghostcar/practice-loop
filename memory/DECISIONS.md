...
| --- | --- | --- | --- | --- |
<!--
ФОРМАТ РЕЕСТРА (канонический, обязателен для новых ADR):
  | ADR-NNN | YYYY-MM-DD | Тема | Краткое решение | статус |
  статус — только: принято | отложено | отклонено.
  Подробная секция «### ADR-NNN — Тема» (именно em-dash и H3) опциональна.
  Пропущенные номера фиксируются комментарием-надгробием: «ADR-NNN: номер не использован».
  Номера без реестровой строки, для которых нет подтверждённого решения, считаются неиспользованными;
  текущие tombstones: ADR-121, ADR-131, ADR-132, ADR-133, ADR-134, ADR-159.
  На tombstone-номера не ссылаются как на принятые решения.
  Парсер tools/memoryctl/adr.py принимает также англ. статусы и варианты, но новые записи
  должны следовать канону. Инструмент: python3 -m tools.memoryctl adr check
-->
...
| ADR-124 | 2026-08-20 | Двухуровневая Библиотека Промптов (System & User Prompts) и Админ-Хаб (/admin/prompts) | Все ИИ-промпты портала централизованы в `PromptLibraryItem` (миграция 073) + регистратор `app/prompt_library.py` | принято |
| ADR-125 | 2026-08-20 | Dynamic Adaptive Training Program Engine & Interactive Hub (/training/adaptive) | Динамические многодневные адаптивные тренировочные программы (миграция 074) + UI-хаб `/training/adaptive` | принято |
| ADR-135 | 2026-08-21 | Передача владения сообществом и со-модераторы (CommunityMemberRole) | Роли участников сообществ через `community_member_roles` (миграция 082): co_top → модератор, передача владения | принято |
| ADR-136 | 2026-08-21 | Рекомпозиция продукта: R0-аудит и 5 документов примирения (Master Brief) | R0-аудит без кода по Master Brief + 5 документов примирения | принято |
| ADR-137 | 2026-08-21 | Лекарства: настраиваемая positive-only геймификация (смягчение PD-013, ADR-085) | Геймификация лекарств — пользовательская политика положительного подкрепления в prefs | принято |
...
| ADR-120 | 2026-08-19 | Quests & Weekly Challenges | Интерактивный хаб квестов и челленджей с моделями Quest/UserQuest, seed-каталогом и страницей `/achievements/quests` | принято |
| ADR-126 | 2026-08-20 | Body State & Cycle Tracker Hub | Логирование фазы цикла, энергии и болезненности через `BodyCycleLog`, UI `/health/body-cycle`, миграция 075 | принято |
| ADR-127 | 2026-08-20 | Equipment & Device Maintenance Log | Owner-scoped журнал очистки, осмотра и обслуживания устройств, UI `/inventory/maintenance`, миграция 076 | принято |
| ADR-128 | 2026-08-20 | D/s Suite & Submissive Management Hub | D/s-профили, keyholder-портал, offline/registered submissives и scoped management через миграцию 077 | принято |
| ADR-129 | 2026-08-20 | Voluntary Delegation & Capability Grants | Добровольное делегирование через CapabilityGrant: 7 scopes, invite/claim/revoke и Safe Word emergency revoke | принято |
| ADR-130 | 2026-08-20 | Unregistered Submissive Telegram Linking | Привязка offline submissive к Telegram-коду и прямые push-уведомления, миграция 080 | принято |
<!-- ADR-121: номер не использован; отдельного решения в истории не найдено. -->
<!-- ADR-131: номер не использован; отдельного решения в истории не найдено. -->
<!-- ADR-132: номер не использован; отдельного решения в истории не найдено. -->
<!-- ADR-133: номер не использован; отдельного решения в истории не найдено. -->
<!-- ADR-134: номер не использован; отдельного решения в истории не найдено. -->
<!-- ADR-159: номер не использован; отдельного решения в истории не найдено. -->
...
> Implementation note (2026-08-31): LLM catalog work is documented in `README.md`; active generated state must be refreshed with `memoryctl facts` after committing.
...
| ADR-145 | 2026-08-21 | Agency, Capability, Dynamic and Protocol engines | В RC v1.0.0-rc1 приняты bounded contexts Agency/Capability/Dynamic/Protocol, миграции 085–088, сервисы и API; все мутации проходят через ActorContext и capability checks, цифровая автономия и safety-stop сохраняются | принято |
...
- **Коммит:** (pending)
| ADR-172 | 2026-08-25 | Service layer extraction: diets.py → diets_service.py (529→386 thin routes, 422 service) | thin routes; коммит 085e9a85 | принято |
| ADR-173 | 2026-08-25 | Service layer extraction: prompt_templates.py → prompt_templates_service.py (505→307 thin routes, 350 service) | thin routes; коммит 8b5c3953 | принято |
| ADR-174 | 2026-08-25 | Service layer extraction: import_data.py → import_data_service.py (493→148 thin routes, 422 service) | thin routes; коммит 8b5c3953 | принято |
| ADR-175 | 2026-08-25 | Service layer extraction: calendar.py → calendar_service.py (426→207 thin routes, 323 service) | thin routes; коммит f6a83229 | принято |
| ADR-176 | 2026-08-25 | Service layer extraction: protocols.py → protocols_service.py (404→230 thin routes, 360 service) | thin routes; коммит 1d5e74b6 | принято |
| ADR-177 | 2026-08-25 | Initial onboarding wizard — 3-step flow (superseded by ADR-184) | feature; коммит b3ea2932 | принято |
| ADR-178 | 2026-08-25 | Service layer extraction: communities/admin/catalog/community_agent/aftercare (5 files, ~1713 lines total) | thin routes; коммит 175457f2 | принято |
| ADR-179 | 2026-08-25 | New-user bootstrap: catalog opt-in and LLM presets | После onboarding система идемпотентно создаёт системный каталог, opt-in для публичных сущностей и пресеты Omniroute/Groq/OpenRouter; deterministic task path доступен сразу после регистрации | принято |
| ADR-181 | 2026-08-25 | OCR-first verification pipeline — local pytesseract + LLM-vision fallback (v0.9.1) | Локальный pytesseract (conf ≥0.90) с LLM-vision фолбэком; timeout 5s; авто-подпись тегов в Media Vault | принято |
| ADR-182 | 2026-08-25 | Platform Social enablement — default ON for v1.0 rollout | social_enabled/public_enabled=True, community_creation_limit=3, 'social' в PROFILE_MODULES | принято |
...
| ADR-187 | 2026-09-04 | Удаление мёртвых экспериментальных таблиц (automation_triggers / user_league_tiers / user_duels) | Три таблицы (миграция 082) никогда не имели роутов/UI/подключения к рантайму; agent-движки — прототипные (хардкод-счёт в дуэлях, авто-штрафы в automation), конфликтовали с «без авто-эскалации» (ADR-106). Таблицы пусты в production. Удалены миграцией 094 (downgrade воссоздаёт схему), модели/agent-файлы и их unit-тесты удалены, config.experimental_leagues убран, emoji-иконки в i18n заменены на икон-пак (ADR-151) с watchdog-тестом. Восстановление — из git-истории. | принято |
| ADR-188 | 2026-09-04 | `alembic check`: полное покрытие моделей + фиксация остаточного schema drift | `alembic/env.py` дополнен до полного импорта моделей (были ложные `remove_table`); исправлены модели в сторону БД: `LockSlotRule` +4 колонки (миграция 068), `TrainingLogEntry.equipment_item_ids` (миграция 069). Остаточный drift (переименование индексов 84/88, JSONB↔JSON 76, FK ondelete 8/10, unique 8/2) зафиксирован как долг S6 — reconciliation-миграция отдельно, по решению владельца; `alembic check` как pre-deploy gate не включать до неё. | принято |
| ADR-190 | 2026-09-04 | Medication: «активный элемент», состав, суточные пределы, замена | Действующее вещество отделяется от МНН → сущность `med_substances` (norm_key UNIQUE, inn, synonyms, daily_max_*; seed из pharma_enricher). Препарат = 1..N компонентов (`med_components` m2m + дозировка) + варианты пачки `med_variants` (Фемостон 2/10: белые 1–14/серые 15–28; поливитамины); `medications.allow_ul_override` — явное разрешение превышения суточной дозы. Группировка/поиск/замена — по веществам. Замена при нехватке: автовыбор при том же наборе компонентов + форме (пересчёт мг) с фиксацией `med_intakes.substituted_for_id` + автотекстом, иначе предложение («Эквиваленты»). Суточные пределы: сверка суммы компонентов дня (с вариантами пачки) vs daily_max, «принять с превышением» (`ul_confirmed`). Автопоиск по наименованию: structured-seed + маска Фемостон N/M + реальный LLM + честный «не найдено». Фазы E–G, спека K-MEDICATION-SUBSTANCES-SPEC; **E–G ✅ (2026-09-04, миграции 097–098)**. | принято |
| ADR-189 | 2026-09-04 | Medication: механизм аптечек, режимов и курсов приёма | Аптечки — отдельная секция + авто-партия из формы препарата; режим = семантические параметры на `MedSchedule` (food_relation, duration_days, meal_timing, meal_offset_min; миграция 095) + пресеты `REGIMEN_PRESETS` и `regimen_to_text`; курсы `MedCourse` (миграция 096: med_courses + `course_id` на расписаниях + `med_kits.location_id` → иерархические TaskLocation) с вычисляемым планом `course_summary` (потребление vs аптечки с локациями, без материализации, MedIntake — факты) и сводными приёмами по слотам в «На сегодня» (`med_group` + batch-intake); умный ввод (`parse_regimen_text` RU/EN + «Разобрать» в формах расписания/элемента курса + `POST /medications/parse-regimen` и JSON `/regimen/parse`, только предзаполнение формы с подтверждением, ничего не сохраняет; LLM-уточнение — опциональный слой). Фазы A–D реализованы 2026-09-04. Спека K-MEDICATION-COURSES-SPEC. | принято |
| ADR-191 | 2026-09-08 | Карточка медикамента, личные LLM в навигации, фикс админ-пула, журнал LLM, слоты приёма | (1) Карточка препарата: якорь `#med-{id}`, кнопка открытия карточки, редактирование состава и параметров. (2) Личные LLM (`/llm-configs`) в главной навигации (ADR-033). (3) Фикс `/admin/llm-pool`: eager-load `provider.models` (MissingGreenlet) + обработка дубликатов. (4) Resolver: распознаёт устаревший portal id `"{id} (local)"` -> фолбэк. (5) Журнал вызовов LLM: миграция `099_llm_call_logs`, страница `/llm-configs/logs` + JSON-parity. (6) Фикс удаления препарата: `Medication.components/variants` cascade delete-orphan, passive_deletes=True. (7) «Как принимать» -> график: `maybe_schedule_from_instructions` авто-создаёт MedSchedule. (8) Объединение приёма по слотам: `summary.slots` с пакетным приёмом `/med-intakes/batch`. Регрессия 1540 passed / 4 skipped. | принято |
| ADR-192 | 2026-09-08 | Модалки аптечек, курсов и сканер DataMatrix (Честный Знак РФ) | (1) Модальные окна редактирования и пополнения аптечек (`MedKit`): добавлены `update_kit`, `add_stock_to_kit` и маршруты `POST /med-kits/{id}/update`, `/add-stock`. (2) Модальные окна курсов (`MedCourse`): добавлены `update_course`, `delete_course_item`, умный разбор режима приёма. (3) Сканирование и парсинг GS1 DataMatrix (Честный Знак РФ / МДЛП) через `zxing-cpp`: разбор AI (01, 21, 17, 10, 91/92), автозаполнение партии, срока годности и GTIN, распознавание в Telegram-боте (`/scan` и фото) с мгновенным добавлением в выбранную аптечку. (4) Парсер и поиск препаратов в онлайн-справочниках (`pharma_online.py` + `pharma_enricher.py`). | принято |
| ADR-193 | 2026-09-08 | Персональный контур Telegram-бота (One-Click, слоты лекарств, AI-генератор, тренировки, замеры, штрафы) | Перенос персонального контура в Telegram-бота: (1) Постоянная нижняя клавиатура `get_main_reply_keyboard()` (6 секций: План дня, Лекарства, AI-генератор, Тренировка, Чек-ин/Замеры, Прогресс). (2) 1-Click выполнение задач дня с плановыми параметрами (`done:{id}`) без лишних окон, поддержка кастомных минут и прерывания со штрафом (ADR-029). (3) Пакетный приём лекарств по слотам («Принять всё в 09:00») и поштучно с начислением XP. (4) AI-генератор активностей через LLM-ассистента с пресетами (auto, quick, relax, intense); ручные сценарии остаются на сайте. (5) Тренировки: чеклист упражнений, завершение дня, авто-адаптация нагрузки. (6) Чек-ин самочувствия (шкалы настроения/энергии 1-5), запись веса через FSM. (7) Прогресс и отработка штрафов (`PenaltyRedemption` -> `PointsTransaction`). | принято |
| ADR-194 | 2026-09-09 | Бесшовный механизм привязки Telegram-бота (1-Click Deep Link, SVG QR-код, двусторонний OTP) | Сервис `telegram_link_service`: (1) 1-Click переход по deep link (`https://t.me/<bot>?start=link_<code>`) с мгновенной привязкой. (2) Векторный SVG QR-код в профиле без внешних библиотек через `zxing-cpp` (`/profile/telegram-qr`). (3) Двусторонний OTP: бот генерирует 6-значный код (`/connect`, `/otp`) на 10 минут для ввода на сайте (`POST /profile/telegram-link-by-otp`), сайт выдает код для команды `/link` в боте. (4) Управление статусом привязки, username/chat_id и кнопка безопасной отвязки (`POST /profile/telegram-unlink` и команда `/unlink` в боте). | принято |
| ADR-195 | 2026-09-09 | Свободный режим таймера верности (Open-Ended Wear), реактивный движок событий и триггеров, раздел «🔒 Пояс» в Telegram-боте | Непрерывный образ жизни без искусственных дедлайнов (`LockSession` `mode="open_ended"`): (1) Справочник событий `wear_event_definitions` (санитарная обработка макс 10 мин, уходовые процедуры/депиляция, спорт, медицина, секс, форс-мажор с четкими критериями, срыв/нарушение, отметка оргазма/эякуляции, инспекция пломбы без снятия) + журнал `wear_event_logs` с точностью до секунды. Любое опоздание возврата — штраф. (2) Интеграция с личным контуром: событие «Секс» авто-создает `JournalEntry`, отметка оргазма/эякуляции фиксируется в сексуальном журнале, уход порождает задачи обработки кожи, дискомфорт облегчает тренировки, нарушения порождают штрафные задачи в плане дня. (3) Агентский режим: свободный ввод причины -> LLM классификация и настройка таймеров. (4) Бот: кнопка «🔒 Пояс», динамическая кнопка Открыть/Закрыть по текущему статусу, таймер с секундами, ближайшие плановые события базового таймера на сегодня. | принято |
| ADR-196 | 2026-09-09 | Ролевая система профиля, двухуровневая таксономия, классификатор состояний и статус-теги | Разделение идентичностей и ролевой классификатор: (1) Иерархия наименований: системное имя `display_name`, портальное ролевое имя `portal_name`, реестровый индекс ИИ `ai_designation`. При `ai_identity_locked=True` ролевое имя и индекс назначаются алгоритмом ИИ однократно и блокируются от ручной правки; ИИ варьирует грамматические формы при обращении. (2) Двухуровневая ролевая таксономия: базовая роль `primary_role` (submissive, dominant, keyholder, switch, sissy, master_mistress) + контекстные роли `portal_roles` (pet, chastity_captive, pain_slut, slave, slut, sissy_maid, brat, service_sub и др.) с проверкой матрицы совместимости. (3) Трехуровневая модель статус-тегов `status_tags` (JSON: permanent — фундаментальные атрибуты, standing — сессионные/циклические, dynamic — реактивные события с устареванием). (4) Физиологический профиль `physiology` (male/female/intersex) изолирован от социо-ролей и используется для расчетов метаболизма, нагрузок и девайсов. (5) Интеграция с режимом Discretion (подавление ролевых наименований). | принято |
| ADR-197 | 2026-09-09 | Редизайн конфигуратора сессии пояса (Draft vs Active, дисциплина, фотоконтроль, Pillory) | (1) Разделение Draft/Active: в черновике только настройки запуска, скрыты блоки ухода и чек-инов (появляются при active), убран чужеродный блок лекарств, таймзона берется из профиля. (2) Дисциплинарный протокол: штрафные очки, штрафное время (+минуты к таймеру), авто-назначение штрафных заданий в план дня, эскалация. (3) Фотоконтроль и верификация: одноразовый буквенно-цифровой код (VerificationChallenge), фото пломбы с кодом на бумаге, фиксация бирки и кода в логах, проверка AI/сообществом/ключником. (4) Интеграция с Позорным столбом (Pillory): отправка при срывах/нарушениях, колесо продления голосами сообщества (+15м/+1ч прямое продление таймера). | принято |
| ADR-198 | 2026-09-09 | Геймификация управления временем сессии пояса (Chaster Extensions, Колесо Фортуны, Кубик Судьбы, Испытания послушания, Скачки времени) | Механизмы Chaster.app в управлении временем: (1) Колесо Фортуны (Wheel of Fortune): секторы добавления времени (+30м..+12ч), милосердия (-15м..-30м), заморозки, ссылки на Pillory, испытания послушания и XP. Кулдаун и анимация. (2) Кубик Судьбы (Dice of Fate 2d6): крит. провал (2: +4ч + столб), неудача (+15м за очко), задание (6..8), милосердие (-10м за очко), джекпот (12: -2ч + 100 XP). (3) Испытания послушания (Obedience Challenges): пикантно-унизительные фото-задания с одноразовым кодом (покаяние на коленях, клеймо на теле, наряд питомца/сисси, поклон котоу, табличка в зубах, раскаяние на столбе); дедлайн, награда временем при сдаче или штрафное время/столб при отказе. (4) Таинственные скачки времени ИИ (Time Jumps). (5) Таблица lock_game_actions для персистентной истории всех игр и спинов сессии. | принято |
| ADR-199 | 2026-09-09 | Перманентная заморозка и разморозка таймера сессии пояса (Permanent Freeze & Unfreeze State, события WearEventDefinition, UI и расширения) | События и механика заморозки таймера: (1) Состояние сессии: `is_frozen`, `frozen_at`, `frozen_remaining_seconds`. При заморозке ход времени замирает, обратный отсчет останавливается на текущем остатке. (2) События WearEventDefinition: `timer_freeze` (перманентная заморозка, trigger_config: freeze_timer) и `timer_unfreeze` (разморозка, trigger_config: unfreeze_timer) с фиксацией в `wear_event_logs`. (3) Разморозка: восстановление отсчета `effective_end_at = now + frozen_remaining_seconds`. (4) Интеграция в Chaster Extensions: секторы Колеса Фортуны «❄️ Перманентная заморозка» и «🔥 Разморозка», снятие заморозки за успешные испытания покорности. (5) UI и Telegram-бот: ледяной статус заморозки, остановка таймера, кнопки ручного управления заморозкой/разморозкой (ADR-129). | принято |
| ADR-200 | 2026-09-09 | Синхронизация статус-тегов пользователя с состояниями таймера пояса и эскалация полосы неудач в мини-играх | Реактивная интеграция статусов: (1) Статусы таймера: при старте сессии присваиваются #chastity_locked и #in_lock_cycle (standing); при заморозке #frozen_timer (standing), при разморозке #thawed (dynamic); при отправке на Позорный столб #pilloried (standing); при испытании #under_trial, при успехе #obedient, при отказе #disobedient; при safety-stop #broken_lock и #punished; при штатном финише #lock_survived. (2) Эскалация полосы неудач в мини-играх (Колесо Фортуны и Кубик Судьбы): учет непрерывной серии плохих исходов bad_luck_streak: 2 неудачи подряд -> динамический статус #unlucky (Полоса неудач); 3 неудачи подряд -> эскалация до условно-постоянного статуса #loser (Неудачник фортуны, standing); 4+ неудач подряд -> высшая эскалация до постоянного статуса #pathetic_loser (Хронический неудачник, permanent). (3) Искупление (Redemption): джекпот в играх или успешная сдача фото-испытания послушания снимают #unlucky и #loser; постоянный статус требует ручного решения или Верхнего (ADR-129). (4) Отображение статус-тегов и бейджей полосы неудач в UI сессии и профиле. | принято |
| ADR-201 | 2026-09-09 | Интеграция статус-информирования, мини-игр Chaster Extensions и фото-подтверждений в Telegram-боте | Интеграция личного контура в Telegram-бот: (1) Информирование и статус-карточка: команда /status и карточка «🔒 Пояс» выводят активные теги пользователя (#chastity_locked, #frozen_timer, #unlucky, #loser, #pathetic_loser), серию неудач bad_luck_streak и бейджи состояния; при эскалации статуса бот отправляет уведомление в чат. (2) Мини-игры в боте: инлайн-кнопки «🎡 Колесо Фортуны», «🎲 Кубик Судьбы (2d6)», «❄️ Заморозить» / «🔥 Разморозить», «🎭 Испытание» прямо под карточкой пояса с наглядной анимацией и модификацией таймера/XP. (3) Фото-подтверждения и испытания в чате: пользователь может сдать фото-испытание послушания с кодом или фотоотчет проверки пломбы прямо отправкой фото в бот (стейты waiting_for_challenge_photo и waiting_for_wear_inspection_photo); бот верифицирует сдачу, начисляет XP, гасит серию неудач и снимает заморозку. (4) Принцип Добровольной Эмуляции (ADR-129): все действия доступны удаленно без обязательного входа на сайт. | принято |
| ADR-202 | 2026-09-09 | Удаление черновиков сессий пояса верности | Удаление невостребованных черновиков сессий таймера пояса: (1) Безопасность: удалять разрешено только сессии в статусе draft; активные, замороженные или завершенные сессии неизменяемы в истории (ADR-029/ADR-197). (2) Очистка: каскадное удаление правил (LockSlotRule, LockTaskRule), снимков (LockSessionSnapshot), дерегистрация SocialSubject (timer.session) и освобождение устройства. (3) API и UI: эндпоинт POST /api/v2/locktimer/sessions/{id}/delete, кнопки удаления с подтверждением в списке черновиков на дашборде /locktimer и на странице черновика /locktimer/sessions/{id}. | принято |
| ADR-203 | 2026-09-09 | Расширенный конфигуратор сессий таймера пояса (человекочитаемые названия, пресеты правил без JSON, гибкий фотоконтроль, Chaster-геймификация и параметры Pillory) | Комплексная модернизация конфигуратора сессии: (1) Идентификация: поля title (пользовательское название) и session_number (порядковый номер) в LockSession, отображение «Сессия #N: Название» вместо UUID. (2) Преднастройки и сохранение: сбалансированные дефолты при создании черновика, кнопка сохранения в панели действий шапки. (3) Режимы: скрытие срока в open_ended; в scheduled — дни/часы и максимальный потолок продления (max_end_at). (4) Правила без JSON: понятные поля в минутах/часах, быстрые чипы и готовые пресеты (утренний душ, гигиена, уход, проверка замка). (5) Фотоконтроль: произвольный ввод часов + быстрые чипы (4ч, 6ч, 8ч, 12ч, 24ч, 48ч). (6) Геймификация: явное подключение Chaster Extensions прямо в черновике. (7) Штрафы и Pillory: каталог штрафных заданий, настраиваемое время продления/помилования за голос сообщества. | принято |
| ADR-204 | 2026-09-09 | Штрафные задания пояса из справочника активностей (Entity), селектор нарушений, эскалация, синглтон-черновик и цифровой код верификации | Комплекс уточнений регламента и эргономики: (1) Верификация: 6-значный криптографический цифровой код (0-9) без букв. (2) История ношения: исключение черновиков из recent sessions (только реальные сессии). (3) Синглтон-черновик: у пользователя не более одного черновика одновременно; кнопка «Новая сессия» и POST /new открывают сохраненный черновик. (4) Штрафные задания из справочника: выбор активностей Entity с привязкой к нарушениям (any, late_return, breach_relapse, task_missed, verification_missed); режимы random_one и all; динамическая эскалация нагрузки при повторах (initial_value * escalation_multiplier^(violation_count-1)); создание реальных ActivityLog в Плане дня. (5) Изоляция логики в app/locktimer/ для совместимости с параллельным рефакторингом модульности. | принято |
| ADR-205 | 2026-09-09 | Локальный OCR-режим верификации таймера пояса и памятка об однозначности кода | Автономная фотоверификация без внешних ИИ: (1) В селектор verification_mode добавлен режим local_ocr («Локальный OCR (Tesseract / без ИИ)») наряду с ai_vision, community, keyholder. Фото обрабатывается локально на сервере (app/media/ocr_seals.py), гарантируя полную приватность, работу без интернета и защиту от safety-блокировок. (2) Контекстная памятка в UI: при выборе local_ocr в черновике и на экране активного контроля выводится предупреждение о необходимости однозначности кода — один крупный код на чистой бумаге маркером или на экране часов/калькулятора; списки со старыми перечеркнутыми кодами запрещены для OCR, так как вызывают графический шум. (3) Принцип Добровольной Эмуляции (ADR-129): сохранение возможности ручного ввода. | принято |
| ADR-206 | 2026-09-09 | Сквозной дисциплинарный контур, мульти-столб (Pillory), 6 ступеней эскалации, сдвиг силы активностей и модульный Telegram-бот | Комплексная сквозная дисциплинарная архитектура: (1) Единый счетчик нарушений и 6 ступеней эскалации (множители от ×1.0 до ×6.0), шаговое искупление (2, 4, 6, 8, 10 успешных проверок подряд для снижения на 1 ступень). (2) Двухуровневый коэффициент: базовый коэффициент строгости в профиле (×1.0 или >2.0) × текущая ступень нарушений. (3) Градация физической силы активностей: каждые 2 ступени нарушений сдвигают интенсивность параметров заданий (light -> medium -> strong/severe). (4) Сквозное влияние на задачи: тумблер в профиле escalation_affects_routine_tasks (влияние на регулярные задачи или только на объем и строгость штрафных). (5) Сквозной Позорный столб (Universal Pillory): независимые инстансы PillorySession под каждый триггер (опоздание фото, провал испытания, сектор колеса, срыв); интеграция с таймером пояса (заморозка + штрафная эскалация заморозки при 4+ продлениях) или автономный медиа-позор; интерактивные кнопки «Усилить / Смягчить»; масштабирование продлений штрафным множителем. (6) Контроль остатков медикаментов: исключение пассивного каталога из напоминаний, запрет уведомлений о приеме при нулевом остатке с разовым запросом пополнения. (7) Модульный Telegram-бот: декомпозиция bot.py на роутеры (auth, tasks, meds, inventory, locktimer, pillory, health, profile), 100% покрытие рутины дня без браузера, пополнение медикаментов и пломб через чат. | принято |
| ADR-207 | 2026-09-10 | Комплексная фармацевтическая экосистема (ИИ-мастер курсов, ситуативные приёмы PRN, мульти-аптечный учёт, группировка Время+Еда и авто-списание FEFO) | Комплексная фармацевтическая экосистема: (1) Автоматическое списание остатков при приёме по принципу FEFO (First Expired, First Out) с учетом аптечки и возможность отмены приёма с возвратом. (2) Ситуативный приём по требованию (PRN): фиксация разовых приёмов (обезболивающие/спазмолитики) с выбором конкретной аптечки списания и причины. (3) Мульти-аптечный учёт и привязка курсов: препарат в разных аптечках (Дом, Офис, Сумка), привязка слота к preferred_kit_id, подсказка «Брать из: Локация». (4) Группировка слотов «Время + Еда»: внутри времени разделение на «до еды за 30 мин», «с едой», «после еды», «независимо», «натощак». (5) ИИ-Мастер курсов (Course Wizard): протокол по цели (ЗГТ лактация, суставы, витамины) с фазами, дозами и сопоставлением остатков. (6) Умный ввод с чипами дозировок, быстрые действия в аналогах и авто-переход при исчерпании оригинала. (7) Telegram Snooze («Отложить на 15/30 мин»). | принято |
| ADR-208 | 2026-09-10 | Бесшовный черновик LockTimer, реальный каталог штрафов, Chaster Extensions Config и безопасное списание аптечек | Комплекс решений по стабильности черновика таймера и точности аптечного учета: (1) Бесшовный черновик LockTimer: AJAX/JSON добавление и удаление окон снятия и заданий без перезагрузки страницы (is_json_requested), динамический монтаж карточек в DOM, автосохранение черновика в localStorage и на сервере. (2) Каталог штрафных заданий: удаление искусственных категорий (physical, chores, reports), полный доступ ко всем 190+ активностям каталога с группировкой по типам (<optgroup>) и русскими метками, автоопределение базовых единиц (мин, раз, удары, мл). (3) Chaster Extensions Suite: развернутые поля конфигурации кулдаунов (Колесо/Кости), флаги опасных секторов (Заморозка, Столб позора), дедлайн испытаний с триггером столба при провале, порог и множитель защиты от неудач. (4) CSP-комплаенс готовых пресетов: переход на data-атрибуты с nonce-скриптом. (5) Аптечный контур: запрет автовыбора аптечки при списании (явное указание пользователем при отсутствии привязки или нулевом остатке), добавление препаратов в аптечку без обязательного пополнения, каскадное удаление запланированных слотов при удалении курса. | принято |
| ADR-209 | 2026-09-10 | Сквозной протокольно-игровой контур: саморабство, расширение границ, слияние сессий (Session Fusion) и режим неизвестности | Архитектурная фиксация целостного игрового и дисциплинарного контура (K-HOLISTIC-BDSM-EXPANSION-PROTOCOL): (1) Добровольное делегирование контроля (ADR-129) и самоподдерживаемость («настроил и живёт само»). (2) Слияние сессий (Session Fusion): гармоничная интеграция долгосрочного фона (пояс верности / режим) и разовых вечерних сессий (бондаж, порка, депривация) с объединением слотов снятия, задач и формированием составного протокола. (3) Режим абсолютной неизвестности (Full Unknownization): скрытый горизонт окончания, плавающий срок, скрытый пул внезапных заданий дня. (4) Программы преодоления границ (Threshold Expansion): структурированные многонедельные курсы прогрессивной перегрузке выносливости ношения, боли и сенсорной изоляции. (5) Сквозная привязка эскалации к инвентарю (сдвиг тяжести инструментов) и обязательный протокол Афтеркея (Aftercare Loop) с эмоциональным чекином. | принято |
| ADR-210 | 2026-09-10 | Кинк-идентичность (BDSMTest, DirtyCode, SexMap), публичная неопределенность толпы (Crowd Governance) и автономный ИИ-Контроль | Комплекс решений по расширению персонального и социального контура (K-KINK-IDENTITY-AND-CROWD-GOVERNANCE): (1) Кинк-идентичность: интеграция результатов внешних тестов BDSMTest.org, DirtyCode.io, HumanSexMap (парсинг/ссылки) в профиль пользователя, публичная карточка границ и ролей (Kink Card). (2) Социальная неопределенность (Crowd Governance): публичные замки и столбы позора с условиями освобождения по кворуму голосов («освободить при 50 голосах ЗА», с дедлайном или без), анонимные штрафы и поощрения от аудитории. (3) Персональный контур: преодоление иллюзии самоконтроля через «Темный алгоритм» (Черный ящик ИИ-Хозяина) — пользователь задает границы и цели, а сценарий каждого дня генерируется динамически без предварительного раскрытия; диалоговое присутствие Персоны в Telegram (UserAgentPersona). | принято |
| ADR-211 | 2026-09-10 | Архитектура KinkOS, экстремальный BDSM-спектр, двухуровневые границы и шкала вербального унижения | Полномасштабное расширение персонального контура (K-EXTREME-BDSM-EXPANSION-AND-KINKOS-ARCHITECTURE): (1) Исчерпывающий BDSM-спектр в каталоге и инвентаре: инвазивные девайсы и анальная адаптация (пробки S-XL, замки на пробки), электростимуляция (E-stim/TENS), осознанные проколы и зажимы (Play Piercing, Clamps), температурные игры (воск, лед), дыхательные практики (маски сопротивления с пульсоксиметрией, кляпы), тяжелый импакт, бондаж с проверкой кровотока каждые 15 мин, сексуальный отказ (эджинг, руиненые оргазмы), бытовое услужение и строки наказания. (2) Двухуровневый шлюз границ (2-Level Boundary Gate): уровень 1 — глобальные Hard Limits в профиле (скрывают табуированные категории); уровень 2 — осознанный вход в скрытые категории с подтверждением пароля и точечным переопределением через UserEntityOptIn с безопасными параметрами. (3) Шкала вербального унижения (Degradation Tiers 0-3): от нейтрального трекера до экстремального унижения/стирания эго в UI и Telegram-боте по личному согласию. (4) Психология катарсиса: протокол растворения эго (замена имени на ID раба, обет молчания), симуляция надзора «Всевидящее Око» (Surprise Drills в Telegram), шлюз в Sub-Space с глубоким Афтеркеем. (5) Модульность KinkOS: независимые флаги модулей для каждого пользователя. | принято |
| ADR-212 | 2026-09-10 | Модульная архитектура KinkOS, комплексные сессии с привязкой к локациям, IoT-устройства и механики накопителей | Расширение архитектуры ядра (K-EXTENSIBLE-KINKOS-ENGINES-AND-COMPLEX-SESSIONS): (1) Открытая модульная архитектура активностей (Extensible Activity Engine): подключение любых существующих и будущих BDSM-активностей через специализированные независимые плагины со своей физикой и валидацией. (2) Комплексные многоэтапные сессии: сквозные сценарии с привязкой к локациям (TaskLocation: подземелье, станок, позорный столб, ванна), ритуалом подготовки (Staging & Prep: проверка инвентаря, освещение, безопасность), фазой действия и деэскалацией (санитария, обслуживание инвентаря, Deep Aftercare). (3) Аппаратная интеграция и мобильное приложение (IoT/BLE): протоколы управления умными игрушками (Buttplug.io, Lovense), тайм-локами (Kitchen Safe, Tuya), электростимуляторами (E-stim BLE) и биометрическими датчиками (SpO2, пульс). (4) Движок накопителей и жидкостей (Fluids & Accumulators): учет объемов (дебет/кредит мл/л), контейнеры из инвентаря, дедлайны наполнения; вотерспортс, задержка мочеиспускания (Desperation), сбор семени, дойка простаты, CEI и санитарное подчинение (подгузники). (5) Сквозная интеграция тела: физическая подготовка (растяжка под бондаж, карательный спорт), пищевая дисциплина (посты, fasting) и эстетика (бритье, клизмы, уход за следами) как священные элементы D/s-протокола. | принято |
| ADR-213 | 2026-09-10 | BDSM-аналитика, мониторинг Sub-Drop, предикторы срывов и индекс целостности протокола | Комплексный аналитический и журнальный контур (K-BDSM-ANALYTICS-AND-SUB-DROP-ENGINE): (1) Расширение аналитического движка (app/analytics/engine.py) на BDSM-домены: кросс-корреляции между воздержанием, болью, сном, стрессом и продуктивностью. (2) Мониторинг Sub-Space и защита от Sub-Drop: фиксация глубины транса (1-5), шкала послушания/сопротивления, радар Sub-Drop через 24ч и 48ч в Telegram; при признаках эмоционального отката — автоматическая блокировка наказаний и включение протокола Deep Aftercare. (3) Предиктор срывов (Relapse Prevention): распознавание предвестников срыва (недосып <6ч + пропуск БАД + рост стресса >4/5) и упреждающая разгрузка до совершения инцидента. (4) Долгосрочные кривые адаптации: динамика болевой терпимости к орудиям, скорость заживления кожи под разными кремами, падение дискомфорта при непрерывном ношении. (5) Submission & Protocol Integrity Score (0-100%): интегральный скоринг точности соблюдения окон гигиены до секунды, рапортов и тренировок; еженедельный аналитический отчет (Protocol Debrief) для Хозяина / Себя. | принято |
...
### ADR-193 — Персональный контур Telegram-бота
**Date:** 2026-09-08
...
### ADR-194 — Бесшовный механизм привязки Telegram-бота
**Date:** 2026-09-09
...
### ADR-195 — Свободный режим таймера верности (Open-Ended Wear), реактивный движок событий и раздел «🔒 Пояс» в боте
**Date:** 2026-09-09
...
### ADR-197 — Редизайн конфигуратора сессии пояса (Draft vs Active, дисциплинарный протокол, фотоконтроль с одноразовым кодом и интеграция с Pillory)
**Date:** 2026-09-09
...
### ADR-202 — Удаление черновиков сессий пояса верности (Draft Deletion)
**Date:** 2026-09-09
...
**Date:** 2026-09-10
**Decision:** Внедрение всеобъемлющей фармацевтической экосистемы:
1. **Автоматическое списание остатков при приёме (FEFO Auto-Deduction):**
   - При фиксации приёма (в вебе или боте) доза списывается из остатка `MedStock` с приоритетом FEFO (сначала партии с более ранним сроком годности).
   - Предупреждение о низком остатке (`low_stock_threshold`).
   - Механизм отмены приёма (Unmark) с возвратом таблеток в запас.
2. **Ситуативный приём по требованию (PRN):**
   - Отдельная форма и меню для разовых приёмов (обезболивающие, жаропонижающие) с указанием дозы, симптома/причины и **конкретной аптечки**, из которой взято лекарство.
3. **Мульти-аптечный учёт:**
   - Хранение препарата в нескольких аптечках одновременно (Дом, Офис, Сумка).
   - Привязка курса/слота к `preferred_kit_id` с визуальной подсказкой локации в карточке слота и боте.
4. **Иерархическая группировка слотов «Время + Еда»:**
   - Разделение внутри времени на «До еды», «Во время еды», «После еды», «Независимо от еды», «Натощак».
5. **ИИ-Мастер курсов (Course Wizard):**
   - Интеллектуальный генератор протоколов по цели (ЗГТ лактация, суставы, витамины) с разбивкой по фазам и созданием в 1 клик.
6. **Умный ввод и бесшовная цепочка аналогов:**
   - Чипы выбора дозировок/форм из справочника, быстрые кнопки добавления аналогов в справочник/аптечку/курс, авто-переход на аналог при исчерпании оригинала.
7. **Telegram Snooze:**
   - Кнопки «Отложить на 15 / 30 минут» в напоминаниях.
**Status:** ✅ принято к реализации.
