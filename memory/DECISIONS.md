...
| ADR-087 | 2026-08-17 | LLM-режим пользователя (safe/expanded) + LLM-разбор анализов | Введён **`prefs.llm_mode`** (safe — default, только факты; expanded — рекомендации/советы/интерпретация). Режим управляется в /settings (LLM mode) и **в будущем влияет на все LLM-блоки и степень влияния LLM на «геймплей»** (задел). Первый потребитель — **LLM-разбор анализов** (`app/llm/pipeline/health.py` + `app/llm/health_prompts.py`): safe — нейтральный пересказ + вопросы врачу; expanded — дополнительно рекомендации (в т.ч. по схеме приёма: активные med_schedules подаются в контекст), всегда с дисклеймером «не врач». Эндпоинты: `POST /health/analyze` (form, redirect с результатом в query — stateless) и `POST /api/v2/health/analyze` (JSON). Usage-трекинг на активном LLMProviderConfig. Комплаенс: режим НЕ обходит safety-фильтры провайдеров и не маскирует контент — только расширяет инструктивную рамку промпта (рекомендации vs факты). 6/6 новых тестов + 26/26 health + 79/79 регрессионных ✅, ruff ✅, i18n 1022/1022. | принято |
