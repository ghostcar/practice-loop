...
| ADR-190 | 2026-09-04 | Medication: «активный элемент» (действующее вещество) + замена при нехватке | Действующее вещество отделяется от МНН и становится канонической сущностью `med_substances` (глобальный справочник: name, norm_key UNIQUE, inn, synonyms; seed из pharma_enricher.LOCAL_PHARMA_SEED); `medications.substance_id` + `medications.inn`; группировка/поиск/замена — по сущности. Замена при нехватке остатка: автовыбор при том же активе + форме (пересчёт дозы в мг, напр. 1×500 вместо 2×250, парсер strength) с фиксацией `med_intakes.substituted_for_id`; иная форма — только предложение с показом эквивалента и остатков. Решение владельца: сущность (не строка) + автовыбор при другой дозировке. Фазы E–G, спека K-MEDICATION-SUBSTANCES-SPEC. | принято |
