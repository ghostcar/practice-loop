...
| ADR-189 | 2026-09-04 | Medication: механизм аптечек, режимов и курсов приёма | Аптечки — отдельная секция + авто-партия из формы препарата; режим = семантические параметры на `MedSchedule` (food_relation, duration_days, meal_timing, meal_offset_min; миграция 095) + пресеты `REGIMEN_PRESETS` и `regimen_to_text`; курсы `MedCourse` (миграция 096: med_courses + `course_id` на расписаниях + `med_kits.location_id` → иерархические TaskLocation) с вычисляемым планом `course_summary` (потребление vs аптечки с локациями, без материализации, MedIntake — факты) и сводными приёмами по слотам в «На сегодня» (`med_group` + batch-intake). Фазы A–C реализованы 2026-09-04; D (умный ввод: rule-парсер + LLM-уточнение, только предзаполнение формы с подтверждением) впереди. Спека K-MEDICATION-COURSES-SPEC. | принято |
