...
| ADR-191 | 2026-09-08 | Карточка медикамента, личные LLM в навигации, фикс админ-пула, журнал вызовов LLM | (1) Карточка препарата: якорь `#med-{id}`, кнопка «Открыть карточку», форма редактирования (включая состав/вещества/`allow_ul_override`) → `update_medication`, редирект на якорь. (2) Личные LLM (`/llm-configs`) — пункт в главной навигации (ADR-033 расширен). (3) Фикс `/admin/llm-pool`: eager-load `provider.models` (MissingGreenlet) + локализованная ошибка дубликата имени. (4) Resolver: распознаёт устаревший portal id `"{id} (local)"` → фолбэк на legacy-конфиг; данные чинятся на лету (проверено на живом PG). (5) Журнал вызовов LLM: миграция `099_llm_call_logs` (метаданные, без raw — ADR-034), запись в `call_llm` и во всех call-сайтах (agent/*, locktimer, pipeline/*); страница `/llm-configs/logs` + JSON-parity `/llm-configs/logs/json` и `/api/v2/llm/logs`. Регрессия 1530 passed / 4 skipped. | принято |
| ADR-191 | 2026-09-08 | Медикаменты: фикс удаления, авто-расписание из «Как принимать», группировка приёма по слотам | (6) **Фикс удаления препарата**: `Medication.components/variants` → `cascade="all, delete-orphan", passive_deletes=True` — ORM больше не пытается обнулить NOT NULL FK (был 500 NotNullViolation); маршрут delete (страница/JSON) ловит ошибки: страница — редирект `?del_err=1&del_detail=…` с баннером (технический текст виден админу), JSON — 500 с detail. (7) **«Как принимать» → график**: `maybe_schedule_from_instructions` — разбор `instructions` через `parse_regimen_text` при создании/редактировании препарата; если режим распознан и расписаний нет — авто-создание MedSchedule (частота/времена/еда/длительность/доза) + баннер `?sched_auto=1`. (8) **Объединение приёма по слотам**: блок «На сегодня» на `/medications` переведён с построчного due на `summary.slots` — все препараты курсов/расписаний с совпадающим временем = одна строка слота с batch «Принять всё» (`/med-intakes/batch`), детали (нехватка/замена/ul) остаются индивидуальными; fallback на due. i18n: `med_delete_error`, `med_schedule_auto_created`, `med_take_all`, `med_slot_any`, `med_autofill` (parity 1769/1769); хардкод в формах препарата убран. Регрессия 1540 passed / 4 skipped. | принято |
