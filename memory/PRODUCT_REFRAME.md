...
## 13. Каталог, персонализация (Fork-on-Opt-In) и читаемость UI

- **Эталонный каталог:** 23 файла из `data/seed/` содержат категории, словарь анатомических зон, инвентарь и параметры. Они импортируются в системный каталог как неизменяемый эталон.
- **Персонализация (Fork on Opt-In):** При добавлении активности в разрешенные создается независимый пользовательский экземпляр (`custom_params`, личные диапазоны времени, привязанный инвентарь, заметки). ИИ и сессии работают по личным границам пользователя. Обновления системного сида безопасны.
- **Читаемость UI:** Никакого сырого JSON в интерфейсе. Параметры выводятся через визуальные чипы (время, нагрузка, анатомические зоны, девайсы). Настройка под себя происходит через удобные поля и слайдеры.

## 14. Снятие избыточных барьеров безопасности (v1.0 Relaxed Gates)

1. **Граница одобрения:** Осознанный opt-in пользователя = полное одобрение (ADR-106). `risk_level` — ориентир для ИИ, а не блокирующий гейт.
2. **Медикаменты и уход:** Разрешено начисление очков дисциплины (adherence XP) и включение приёма в шаги Протоколов подготовки/восстановления.
3. **2FA / PIN Shield:** Кэширование доступа к Media Vault на 15–30 минут в сессии браузера.
4. **Двухуровневый D/s-контроль:** Свободная цифровая эмуляция (Soft) vs эмуляция с записью нарушения и штрафом (Strict Audit).
